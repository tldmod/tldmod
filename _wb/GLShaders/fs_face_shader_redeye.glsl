...
void main()
{
  vec4 __sampled = texture2D(diffuse_texture, Tex0);

  vec4 tex_col_1;
  vec4 tmpvar_2;
  tex_col_1 = ((texture2D (diffuse_texture_2, Tex0) * Color.w) + (__sampled * (1.0 - Color.w)));
  ...
  tmpvar_2.rgb += vec3(2.0, 0.5, 0.0) * EyeColor * __sampled.a;
