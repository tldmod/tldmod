  ...
  (4.15,[ #unpaid troops leaving, desertion, prisoners escape
    (try_begin),
        ...
        (try_begin),
            ...
            (store_sub, ":morale_delta", ":desertion_check", ":morale"),
            (val_clamp, ":morale_delta", 5, 20),
            (party_get_num_companions, ":to_remove", p_main_party), #remove X% of troops per tick
            (val_mul, ":to_remove", ":morale_delta"),
  ...
  (0.5,[(call_script,"script_get_region_of_party", "p_main_party"),
      ...
      (try_begin),
        ...
        (try_begin),
          ...
          (display_log_message, "@You have left {s8}."),
        (try_end),
      (else_try),
        ...
        (try_begin),
          (gt, reg1, -1),
          (str_store_faction_name, s9, reg1),
          (display_log_message, "@You are entering {s8} ({s9})."),
        (else_try),
          (display_log_message, "@You are entering {s8}."),
        (try_end),
      (try_end),
 ...
 #MB Version - Native
        ...
        (try_begin),
          ...
          (try_begin),
            ...
            (try_begin),
              ...
              (else_try),
                (eq, ":cur_quest", "qst_oath_of_vengeance"),
                (jump_to_menu, "mnu_oath_quest_fail"),
                (call_script, "script_abort_quest", ":cur_quest", 1),
              (else_try),
                (eq, ":cur_quest", "qst_oath_personal"),
                (jump_to_menu, "mnu_oath_quest_fail"),
                (call_script, "script_abort_quest", ":cur_quest", 1),
              (else_try),
                (call_script, "script_abort_quest", ":cur_quest", 1),
  ...
  (0.5,[
      ...
      (try_begin),
        ...
        (neg|check_quest_concluded, "qst_oath_of_vengeance"),
        (neg|check_quest_succeeded, "qst_oath_of_vengeance"),
        (quest_get_slot, ":target_amount", "qst_oath_of_vengeance", slot_quest_target_amount),
        (str_clear, s4),
        (str_clear, s5),
        (gt, "$oath_kills", 0),
        (store_mul, ":oath_score", "$oath_kills", 100),
        (store_div, ":progress", ":oath_score", ":target_amount"),
        (quest_set_slot, "qst_oath_of_vengeance", slot_quest_current_state, ":progress"),
        
        (try_begin),
            (le, ":progress", 33),
            (str_store_string, s4, "@Your progress to fulfill your oath is considered not very great. (Progress less than a third.)"),
        (else_try),
            (le, ":progress", 66),
            (str_store_string, s4, "@You have made some progress to fulfill your oath, but there is still a long way to go. (Progress about half.) ^^ The enemies are starting to look out for you."),
        (else_try),
            (le, ":progress", 85),
            (str_store_string, s4, "@You have won the hatred of your enemies, press on further and you will fulfill your oath! (Progress more than two thirds.) ^^ The enemies are starting to dispatch additional parties around their fortresses and camps."),           
        (else_try),
            (le, ":progress", 99),
            (str_store_string, s4, "@You have made yourself a name as one of the enemy's worst foes. Your oath is almost fulfilled!"),
        (else_try),
            (gt, ":progress", 99),
        ...
        (try_begin),
          ...
          (neg|faction_slot_eq, "fac_gundabad", slot_faction_state, sfs_active),
          (str_store_string, s23, "@King Dain values your effort to fulfill your oath, but acknowledges that events unfolded too quickly. As a result, "),
          (try_begin),
            (ge, ":progress", 66),
            ...
            (call_script, "script_succeed_quest", "qst_oath_of_vengeance"), #triggers Dain dialog and oathkeeper trait
          (else_try),
            (is_between, ":progress", 66, 85),
            (call_script, "script_finish_quest", "qst_oath_of_vengeance", ":progress"),
          (else_try),
        ...
        (str_store_string, s5, "@Oath Score: {reg65} (Counter refreshes every hour)"),
        
        (add_quest_note_from_sreg, "qst_oath_of_vengeance", 3, s4, 0),
        (add_quest_note_from_sreg, "qst_oath_of_vengeance", 4, s5, 0),
      (try_end),
      
      #Report number of kills for the kill quest
      (try_begin),
        (check_quest_active, "qst_kill_quest_faction"),
        #(neg|check_quest_concluded, "qst_kill_quest_faction"),
      ...
      (try_begin),
        ...
        #(neg|check_quest_concluded, "qst_kill_quest_troop"),
        (quest_get_slot, ":target_faction", "qst_kill_quest_troop", slot_quest_target_faction),
        (quest_get_slot, ":target_troop", "qst_kill_quest_troop", slot_quest_target_troop),
        ...
        (add_quest_note_from_sreg, "qst_kill_quest_troop", 2, s9, 0),
      (else_try),
        (check_quest_active, "qst_kill_quest_troop"),
        (neg|check_quest_concluded, "qst_kill_quest_troop"),
        (quest_get_slot, ":target_faction", "qst_kill_quest_troop", slot_quest_target_faction),
      ...
      (try_begin),
        ...
        (quest_get_slot, ":quest_giver", "qst_kill_quest_bandit", slot_quest_giver_troop),
        (str_store_troop_name, s8, ":quest_giver"),
        (quest_get_slot, ":quest_giver_center", "qst_kill_quest_bandit", slot_quest_target_center),
        (str_store_party_name, s9, ":quest_giver_center"),
        (quest_get_slot, reg67, "qst_kill_quest_bandit", slot_quest_target_amount),
        ...
        (add_quest_note_from_sreg, "qst_kill_quest_bandit", 2, s11, 0),
      (try_end),
...
(12, 
  ...
  [
    ...
    (try_begin),
      ...
      (assign, ":party_count", 0),
      (try_for_parties, ":cur_party"), #add a mass check here, so the trigger doesn't go overboard
        (party_get_template_id, ":cur_party_template", ":cur_party"),
        (eq, ":cur_party_template", ":party_template"),
        (store_distance_to_party_from_party, ":distance", ":cur_party", ":target_center"),
        (le, ":distance", 20),
        (val_add, ":party_count", 1),
      (try_end),
      (le, ":party_count", 4),
      (set_spawn_radius, 7),
      ...
      (party_set_ai_object, reg0, ":target_center"),
    ...
    (try_begin),
	  ...
	  (try_for_range, ":unused", 0,":parties_to_spawn"),
        ...
        (party_set_ai_object, reg0, ":target_center"),
        (party_set_ai_behavior, reg0, ai_bhvr_patrol_location),
        (party_set_ai_patrol_radius, reg0, 3),
    ...
    (try_begin),
      ...
      (try_for_range, ":center", centers_begin, centers_end),
        (eq, ":break", 0),
        (store_faction_of_party, ":center_fac", ":center"),
        (this_or_next|eq, ":center_fac", "fac_moria"),
        (eq, ":center_fac", "fac_gundabad"),
        (store_distance_to_party_from_party, ":distance", "p_main_party", ":center"),
        (le, ":distance", 25),
        (faction_get_slot, ":fac_capital", ":center_fac", slot_faction_capital),
        (store_random_in_range, ":rand", 0, 100),
        (gt, ":progress", ":rand"),
        (try_begin),
            (le, ":progress", 85),
            (party_get_slot, ":target_template", ":fac_capital", slot_center_spawn_patrol),
            (gt, ":target_template", 1),
            (call_script,"script_cf_spawn_around_party_on_walkable_terrain",":center", ":target_template",3), 
            (assign, ":patrol_party", reg0),
            (party_set_faction, ":patrol_party", ":center_fac"),
