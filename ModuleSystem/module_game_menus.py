...
#	(assign, "$sorcerer_quest", 3),
    ...
    ("next_rescue_scene", [], "_",  
		...
		("pick_troops1", [(neg|quest_slot_eq, "qst_mirkwood_sorcerer",slot_quest_current_state,3)], "Pick companions.",
							[(party_get_num_companion_stacks, ":num_stacks","p_main_party"),
							(try_for_range, ":slot", 0, 40),
								(troop_set_slot,"trp_temp_array_a",":slot",-1), # clear array
							(try_end),
							(assign,":slot",1),
							(try_for_range, ":stack_no", 0, ":num_stacks"), # store troops eligible for stealth
							  (party_stack_get_troop_id,":stack_troop","p_main_party",":stack_no"),
							  (neq, ":stack_troop", "trp_player"), #omit player
							...
							#	(party_stack_get_size,":stack_size","p_main_party",":stack_no"),
								...
								(val_add,":slot",1),
							(try_end),
							(assign,reg1,0),(assign,reg2,0),(assign,reg3,0),(assign,reg4,0),(assign,reg5,0),(assign,reg6,0),(assign,reg7,0),(assign,reg8,0),(assign,reg9,0),(assign,reg10,0),
...
( "pick_troops", 0, 
  "Whom would you take with you into the stealth mission?", "none",
[], [
  ("troop1", [(troop_get_slot,":troop","trp_temp_array_a",1),(gt,":troop",-1),(str_store_troop_name, s1, ":troop")],
   "{s1}: {reg1}", [(troop_get_slot,":troop","trp_temp_array_a",1),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg1,":n"),(val_add, reg1,1),
					 (else_try),(ge,reg1,":n"),(assign,reg1,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop2", [(troop_get_slot,":troop","trp_temp_array_a",2),(gt,":troop",-1),(str_store_troop_name, s2, ":troop")],
   "{s2}: {reg2}", [(troop_get_slot,":troop","trp_temp_array_a",2),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg2,":n"),(val_add, reg2,1),
					 (else_try),                 (assign,reg2,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop3", [(troop_get_slot,":troop","trp_temp_array_a",3),(gt,":troop",-1),(str_store_troop_name, s3, ":troop")],
   "{s3}: {reg3}", [(troop_get_slot,":troop","trp_temp_array_a",3),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg3,":n"),(val_add, reg3,1),
					 (else_try),                 (assign,reg3,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop4", [(troop_get_slot,":troop","trp_temp_array_a",4),(gt,":troop",-1),(str_store_troop_name, s4, ":troop")],
   "{s4}: {reg4}", [(troop_get_slot,":troop","trp_temp_array_a",4),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg4,":n"),(val_add, reg4,1),
					 (else_try),                 (assign,reg4,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop5", [(troop_get_slot,":troop","trp_temp_array_a",5),(gt,":troop",-1),(str_store_troop_name, s5, ":troop")],
   "{s5}: {reg5}", [(troop_get_slot,":troop","trp_temp_array_a",5),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg5,":n"),(val_add, reg5,1),
					 (else_try),                 (assign,reg5,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop6", [(troop_get_slot,":troop","trp_temp_array_a",6),(gt,":troop",-1),(str_store_troop_name, s6, ":troop")],
   "{s6}: {reg6}", [(troop_get_slot,":troop","trp_temp_array_a",6),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg6,":n"),(val_add, reg6,1),
					 (else_try),                 (assign,reg6,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop7", [(troop_get_slot,":troop","trp_temp_array_a",7),(gt,":troop",-1),(str_store_troop_name, s7, ":troop")],
   "{s7}: {reg7}", [(troop_get_slot,":troop","trp_temp_array_a",7),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg7,":n"),(val_add, reg7,1),
					 (else_try),                 (assign,reg7,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop8", [(troop_get_slot,":troop","trp_temp_array_a",8),(gt,":troop",-1),(str_store_troop_name, s8, ":troop")],
   "{s8}: {reg8}", [(troop_get_slot,":troop","trp_temp_array_a",8),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg8,":n"),(val_add, reg8,1),
					 (else_try),                 (assign,reg8,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop9", [(troop_get_slot,":troop","trp_temp_array_a",9),(gt,":troop",-1),(str_store_troop_name, s9, ":troop")],
   "{s9}: {reg9}", [(troop_get_slot,":troop","trp_temp_array_a",9),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg9,":n"),(val_add, reg9,1),
					 (else_try),                 (assign,reg9,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("troop10", [(troop_get_slot,":troop","trp_temp_array_a",10),(gt,":troop",-1),(str_store_troop_name, s10, ":troop")],
   "{s10}: {reg10}", [(troop_get_slot,":troop","trp_temp_array_a",10),
                    (party_count_members_of_type, ":n", "p_main_party",":troop"),
					(try_begin),(lt,reg10,":n"),(val_add, reg10,1),
					 (else_try),                  (assign,reg10,0),
					(try_end),(jump_to_menu, "mnu_pick_troops")]),  
  ("go_forward",   [], "Go forward!",  [#fill in faction companions
