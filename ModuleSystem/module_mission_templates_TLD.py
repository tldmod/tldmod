...
if(is_a_wb_mt):
  ...
  from module_mission_templates_form_v5 import *
...
custom_troll_hitting_new = ((is_a_wb_mt==1) and [
 ...
 # #trolls spawn trigger - overwrite speed and aggressiveness, so disable
 ...
 # ]),
  ...
  [  (try_for_range, ":slot", 0, 1000),
        (troop_set_slot, "trp_array_trolls", ":slot", -1),
     (try_end),
  ]),
  ...
  [  (store_trigger_param_1, ":agent_no"),
     ...
     (try_for_range, ":slot", 0, ":upper_range"),
        (troop_slot_eq, "trp_array_trolls", ":slot", -1),
        (troop_set_slot, "trp_array_trolls", ":slot", ":agent_no"),
        (agent_set_slot, ":agent_no", slot_agent_array_troop, "trp_array_trolls"),       
        (agent_set_slot, ":agent_no", slot_agent_array_number, ":slot"),
        (assign, ":upper_range", 0),

        (assign, reg77, ":slot"),
        (assign, reg78, ":agent_no"),
  ...
  (ti_on_agent_killed_or_wounded, 0, 0, [], [
     (store_trigger_param_1, ":agent_no"),
     (agent_slot_eq, ":agent_no", slot_agent_array_troop, "trp_array_trolls"),
     (agent_get_slot, ":array_number", ":agent_no", slot_agent_array_number),
     (troop_set_slot, "trp_array_trolls", ":array_number", -1),
  ]),

#troll regular attack
(ti_on_agent_hit, 0, 0, [
    ...
    (is_between, reg0, "itm_troll_weapon_long", "itm_troll_aoe"),
 ...
 # 			(agent_deliver_damage_to_agent_advanced, <destination>, <attacker_agent_id>, <agent_id>, <value>, [weapon_item_id]),
	...
	(get_player_agent_no, ":player"),
	(eq, ":receiver", ":player"), #only for player: damaged was halfed above
	#(display_message, "@player {s5} hit"),
	(set_trigger_result, ":damage"),

]),

# InVain: Surround pushback new
  (4, 1.2, 0, [ 
  (ge, "$trolls_in_battle",1),
  (set_fixed_point_multiplier, 100),
  (assign, ":troll_found", 0),
  (try_for_range, ":slot", 0, 100),
    (le, ":troll_found", 2), #not more than 2 trolls strike at a time
    (troop_get_slot, ":troll", "trp_array_trolls", ":slot"),
    (neq, ":troll", -1),
    (agent_is_active, ":troll"),
    (agent_is_alive, ":troll"),
    (agent_is_human, ":troll"),
    (agent_get_troop_id, ":troop_no", ":troll"), #bunch of extra checks just to be sure
    (troop_get_type, ":race", ":troop_no"),
    (eq, ":race", tf_troll),
    (agent_slot_eq, ":troll", slot_agent_troll_status, 1),
    # (assign, reg77, ":slot"),
    # (assign, reg78, ":troll"),
    # (str_store_agent_name, s5, ":troll"),
    (agent_set_slot, ":troll", slot_agent_troll_status, 2),
    (agent_set_animation, ":troll", "anim_troll_pushback"), #todo: measure animation length and adjust trigger consequence timer
  ...
  (try_for_range, ":slot", 0, 100),
    ...
    (troop_get_type, ":race", ":troop_no"),
    (eq, ":race", tf_troll),
    (agent_slot_eq, ":troll", slot_agent_troll_status, 2),
    ...
    (try_for_agents, ":troll", pos69, 500), #morale effect
        ...
        (agent_get_slot, ":morale_penalty", ":troll", slot_agent_morale_modifier),
        (val_sub, ":morale_penalty", 10),
        (agent_set_slot, ":troll", slot_agent_morale_modifier, ":morale_penalty"),
    (try_end),
  (try_end),
...
#InVain Troll Charge new
# troll charges only happen if target agent is within charge range. This should mostly be the case when two formations approach each other.
  ...
  (try_for_range, ":slot", 0, 100),
    (lt, ":troll_found", 1), #not more than 1 troll charges at a time
    (troop_get_slot, ":troll", "trp_array_trolls", ":slot"),
    (neq, ":troll", -1),
    (agent_is_active, ":troll"),
    (agent_is_alive, ":troll"),
    ...
    (troop_get_type, ":race", ":troop_no"),
    (eq, ":race", tf_troll),
    (agent_slot_eq, ":troll", slot_agent_troll_status, 0),
    (agent_get_position, pos5, ":troll"),
    # (agent_ai_get_num_cached_enemies, ":num_targets", ":troll"),
    # (try_for_range, ":cached_enemy_no", 0, ":num_targets"),
        # (eq, ":target_found", 0),
        # (agent_ai_get_cached_enemy, ":target", ":troll", ":cached_enemy_no"),
        # (agent_is_active, ":target"),
        # (agent_is_alive, ":target"),
        # (agent_get_position, pos6, ":target"),
        # (get_distance_between_positions, ":dist", pos5, pos6),
        # (is_between, ":dist", 900, 1200), #animation's charge range is ~10m
        # (assign, ":target_found", 1),
    # (try_end),
    # (eq, ":target_found", 1),
    (agent_ai_get_move_target, ":target", ":troll"),
    (agent_is_active, ":target"),
    (agent_is_alive, ":target"),
    (agent_get_position, pos6, ":target"),
    (get_distance_between_positions, ":dist", pos5, pos6),
    (is_between, ":dist", 800, 1300), #animation's charge range is ~10m
  ...
  (try_for_range, ":slot", 0, 100),
    (troop_get_slot, ":troll_agent", "trp_array_trolls", ":slot"),
    (agent_is_active, ":troll_agent"),
    (agent_is_alive, ":troll_agent"),
    (agent_is_human, ":troll_agent"),
    (agent_get_troop_id, ":troop_no", ":troll_agent"), #bunch of extra checks just to be sure
    (troop_get_type, ":race", ":troop_no"),
    (eq, ":race", tf_troll),
    (agent_slot_eq, ":troll_agent", slot_agent_troll_status, 3),
    (agent_set_slot, ":troll_agent", slot_agent_troll_status, 0),
    (agent_clear_scripted_mode, ":troll_agent"),
    (agent_force_rethink, ":troll_agent"),
...
#Troll kicking
  ...
  (assign, ":troll_found", 0),
  (try_for_range, ":slot", 0, 100),
    (lt, ":troll_found", 2), #not more than 2 trolls kick at a time
    (troop_get_slot, ":troll", "trp_array_trolls", ":slot"),
    (neq, ":troll", -1),
    (agent_is_active, ":troll"),
    (agent_is_alive, ":troll"),
    ...
    (agent_slot_eq, ":troll", slot_agent_troll_status, 0),
    (agent_get_position, pos5, ":troll"),
    (agent_ai_get_cached_enemy, ":target", ":troll", 0),
    (agent_is_active, ":target"),
    (agent_is_alive, ":target"),
    (agent_get_position, pos6, ":target"),
    (get_distance_between_positions, ":dist", pos5, pos6),
    (lt, ":dist", 200),
    (neg|position_is_behind_position, pos5, pos6),
    (store_random_in_range, ":chance", 0, 10), #some randomization
    (le, ":chance", 5),
    ...
    (try_for_agents, ":mount"),
		...
		(try_begin),
			...
			(else_try), 						#sounds for dying horses
                (neg|agent_is_alive, ":mount"), #Invain: This check is necessary so it does not trigger on spawn
        ...
        (else_try),
			...
			(try_begin), 	
				(agent_is_alive, ":mount"),
				(store_random_in_range, ":random", 1, 101), 
				(try_begin),(le, ":random", 7),(agent_play_sound, ":mount", "snd_bear_strike"),(try_end),
			(else_try),
                (neg|agent_is_alive, ":mount"),
				(agent_slot_eq, ":mount", slot_agent_mount_dead, 0),
...
#TLD new animal attacks begin
tld_animal_attacks =  ((is_a_wb_mt==1) and [ 
  ...
  [  (store_trigger_param_1, ":agent"),
     ...
     (try_for_range, ":slot", 1, ":upper_range"), #assign to array, leave out 0, because that would be the default slot value for slot_agent_array_number
        ...
        (agent_set_slot, ":agent", slot_agent_array_number, ":slot"),
        (assign, ":upper_range", 0),
        # (assign, reg78, ":slot"),
  ...
  (try_for_range, ":slot", 1, ":upper count"),
    ...
    (agent_is_alive, ":agent"),
    (agent_is_human, ":agent"),
    (agent_slot_eq, ":agent", slot_agent_animal_is_striking, 0),
    (agent_get_troop_id, ":agent_trp", ":agent"),
    ...
    (ge, ":horse", 0),
    (agent_get_animation, ":current_anim", ":horse"),
    (neq, ":current_anim", "anim_horse_rear"), #don't initiate an attack if just taken damage
    ...
    (try_for_range_backwards, ":nearby_agent_no", 0, ":num_nearby_agents"), #checking closest enemies from farthest to closest
    ...
    (else_try),
      ...
      (try_begin),
        (le, ":rnd_2", 25),
        (assign, ":anim", "anim_bear_slap_right"),
        (agent_set_slot, ":agent", slot_agent_animal_is_striking, 110),
      (else_try),
        (le, ":rnd_2", 50),
        (assign, ":anim", "anim_spider_attack"), #works for bear too
        (agent_set_slot, ":agent", slot_agent_animal_is_striking, 130),
      (else_try),
        (le, ":rnd_2", 75),
        (assign, ":anim", "anim_bear_uppercut"),
        (agent_set_slot, ":agent", slot_agent_animal_is_striking, 150),      
      (else_try),
        (assign, ":anim", "anim_bear_slam"),
        (agent_set_slot, ":agent", slot_agent_animal_is_striking, 180),
      (try_end),
    (else_try),
  ...
  (try_for_range, ":slot", 1, ":upper count"), #circle through slot array
    ...
    (agent_is_active, ":agent"),
    (agent_is_alive, ":agent"),
    (agent_get_slot, ":attack_range", ":agent", slot_agent_animal_is_striking),
    (gt, ":attack_range", 0),
    (agent_get_troop_id, ":agent_trp", ":agent"),
    (agent_get_horse, ":horse", ":agent"),
    ...
    #damage
    (store_skill_level, ":damage", ":agent_trp", skl_power_strike), #0-10
    ...
    (assign, ":hit_anim", -1),
    #Damaging Enemies  
    (try_for_agents, ":enemy_agent",  pos2, ":attack_range"),
      (agent_is_alive, ":enemy_agent"),
      ...
      (gt, ":enemy_agent", 0),
      # (str_store_agent_name, s5, ":enemy_agent"),
      # (assign, reg78, ":enemy_agent"),
      ...
      (try_begin), #check for blocks
        (agent_is_human, ":enemy_agent"), #just in case
        (agent_get_wielded_item, ":shield", ":enemy_agent", 1),
        (ge, ":shield", 1),
        (agent_get_defend_action, ":defend_action", ":enemy_agent"),
        (gt, ":defend_action", 0), #blocking or parrying
        (neg|position_is_behind_position, pos2, pos3), #not attacked from behind
        (agent_play_sound, ":enemy_agent", "snd_shield_hit_metal_wood"),
        (assign, ":hit_anim", "anim_defend_shield_parry_1"),
      (else_try), #apply damage and animation
        (gt, ":damage", 1),
        (try_begin), #player cheat 
            (eq, ":enemy_agent", ":player_agent"),
            (val_div, ":damage", 2),
        (try_end),
        (agent_deliver_damage_to_agent, ":agent", ":enemy_agent", ":damage", "itm_uruk_falchion_a"), #just a random slashing weapon
        # (str_store_agent_name, s5, ":agent"),
        # (assign, reg78, ":damage"),
        # (display_message, "@{s5} dealt {reg78} damage!"),
        # (agent_get_damage_modifier, reg66, ":agent"),
        # (display_message, "@damage mod is {reg66}"),
        ...
        (val_add, ":timer", 6),
        (agent_set_slot, ":enemy_agent", slot_agent_last_knockdown_time, ":timer"),
      ...
      (agent_is_human, ":enemy_agent"), #needed here so we don't try to set human animation to a horse
      (gt, ":hit_anim", 0),
      (agent_set_animation, ":enemy_agent", ":hit_anim"),
    (try_end),
    (agent_set_slot, ":agent", slot_agent_animal_is_striking, 0),
  (try_end),
  ]),
...
#InVain: Moved from field AI triggers. Not sure if needed. We could simply reduce hit points for lone wargs
(ti_on_agent_hit, 0, 0, [
    (store_trigger_param_1, ":agent"),
...
# (ti_on_agent_hit, 0, 0, [ #for testing
    # (store_trigger_param_1, ":agent"),
    # (get_player_agent_no, ":player"),
    # (eq, ":agent", ":player"),
  # ],
  ...
  # [
    # #(store_trigger_param_1, ":agent"),
    # (store_trigger_param_3, reg78),
        # #(str_store_agent_name, s5, ":agent"),
        # #(assign, reg78, ":damage"),
        # (display_message, "@onhit: player hit for {reg78} damage!"),
  # ]),
  
	] or [])
# TLD new animal attacks end
