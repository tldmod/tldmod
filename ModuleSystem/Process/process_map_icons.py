...
def save_map_icons(variable_list,variable_uses,tag_uses,quick_strings):
  ...
  for map_icon in map_icons:
    triggers = []
    if (len(map_icon) >= 8):
      ofile.write("%s %d %s %s %d %s %s %s "%(map_icon[0],map_icon[1],map_icon[2],sf(map_icon[3]),map_icon[4],sf(map_icon[5]),sf(map_icon[6]),sf(map_icon[7])))
      if (len(map_icon) == 9):
        triggers = map_icon[8]
    else:
      ofile.write("%s %d %s %s %d 0 0 0 "%(map_icon[0],map_icon[1],map_icon[2],sf(map_icon[3]),map_icon[4]))
      if (len(map_icon) == 6):
        triggers = map_icon[5]
