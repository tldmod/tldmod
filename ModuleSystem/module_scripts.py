...
#############################  TLD PLAYER REWARD SYSTEM --- SCRIPTS   (mtarini)  #############################?#
 ...
 # gives to player the income of his rank
  ...
  ("get_own_rank_title",[
	...
	(try_begin),
		(ge, reg10, 9), (str_store_string, s24, "@Uber-Master Knight of {s5} (rank {reg10})"),
	(else_try),
  ...
  ("get_allied_rank_title",[
	...
	(else_try),
		(ge, reg10, 6), (str_store_string, s24, "@dearest friend {s5}"),
	(else_try),
		(ge, reg10, 5), (str_store_string, s24, "@trusted friend of {s5}"),
	(else_try),
		(ge, reg10, 4), (str_store_string, s24, "@admired friend of {s5}"),
	(else_try),
		(ge, reg10, 3), (str_store_string, s24, "@friend of {s5}"),
	(else_try),
		(ge, reg10, 2), (str_store_string, s24, "@popular in {s5}"),
	(else_try),
		(ge, reg10, 1), (str_store_string, s24, "@known in {s5}"),
	(else_try),
		              (str_store_string, s24, "@unknown stranger in {s5}"),
	(end_try),
  ]),
...
("find_closest_enemy_town_or_mayor_party",[
  ...
  ("make_player_pay_upkeep",
	...
	(try_begin),(gt, ":n_tot_unpaid_troops", 0),
		(display_message, "@Short of Resource Points!!", 0xff0000),
		(display_message, "@{s10} will soon reassign some of {s12} troops away from your battle-group!",0xff0000),
	(end_try),
	...
	(try_for_range, ":fac", kingdoms_begin, kingdoms_end),
		...
		(try_for_range_backwards, ":i", 0, ":num_stacks"),
			...
			(try_begin),(eq, ":msg_shown", 0),
				(str_store_faction_name,s11,":fac"),
				(display_message, "@Superior orders from {s11} reassign troops:",0xff0000),
			...
			(display_message, "@{reg12} {s10} left the party!",0xff4444),
 ...
 #############################  TLD FANGORN SCRIPTS   (mtarini)  #############################?#
  ...
  ("after_fangorn_damage_to_player",
   [ (try_begin),
       ...
       (gt,reg1,0),
       (display_message,"@Fangorn claimed {reg0} kills and {reg1} wounded among troops!", 0xFFFF2222),
     (try_end),
     (try_begin),
       (eq,reg2,1),
       (display_message,"@You were wounded in Fangorn!", 0xFFFF2222),
     (try_end),
  ...
  ("party_prisoners_add_party_companions",
    [
      ...
      (try_for_range, ":stack_no", 0, ":num_stacks"),
        ...
        (neg|eq,":race",tf_lothlorien),
        (neg|eq,":race",tf_rivendell),
        (neg|eq,":race",tf_mirkwood),
        (party_stack_get_size, ":stack_size",":source_party",":stack_no"),
...
##  player party temporary relocation to map Z=0, anti-crazy-hills. GA
	 ...
	 (try_begin),  # non-forest types randomly mashed for generation testing purposes
        ...
        (try_begin),
		   ...
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_steppe",0),
        (else_try),
		   (eq, ":terrain", 1),
		   (assign, ":scene_to_use", "scn_random_scene_plain" ),
		   (display_message,"@SCENE: plain"),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_plain",0),
        (else_try),
		   (eq, ":terrain", 2),
		   (assign, ":scene_to_use", "scn_random_scene_snow"  ),
		   (display_message,"@SCENE: snow"),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_snow",0),
        (else_try),
		   (eq, ":terrain", 3),
		   (assign, ":scene_to_use", "scn_random_scene_desert"),
		   (display_message,"@SCENE: desert"),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_desert",0),
        (try_end),
	# get number of combatants (calculated in script_calculate_battle_advantage) and use small scene if too few. GA
	  ...
	  (else_try),  # forest types randomly mashed for generation testing purposes, forests always small, for fps. GA
        ...
        (try_begin),
		   ...
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_steppe_forest",0),
        (else_try),
		   (eq, ":terrain", 1),
		   (assign, ":scene_to_use", "scn_random_scene_plain_forest" ),
		   (display_message,"@SCENE: plain forest" ),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_plain_forest" ,0),
        (else_try),
		   (eq, ":terrain", 2),
		   (assign, ":scene_to_use", "scn_random_scene_snow_forest"  ),
		   (display_message,"@SCENE: snow forest"  ),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_snow_forest"  ,0),
        (else_try),
		   (eq, ":terrain", 3),
		   (assign, ":scene_to_use", "scn_random_scene_desert_forest"),
		   (display_message,"@SCENE: desert forest"),
		   (party_relocate_near_party,"p_main_party", "p_pointer_z_0_desert_forest",0),
        (try_end),
      (try_end),
...
#       (try_end),
       ...
       (try_begin),
         ...
         (try_for_range,":item_no","itm_horse_meat","itm_club"),
...
#     
#     (try_begin),
#       (store_num_parties_of_template, ":num_parties", "pt_lothlorien_war_party"),
#       (lt,":num_parties",2),
#       (store_random,":spawn_point",num_sea_raider_spawn_points),
#       (val_add,":spawn_point","p_gondor_test"),
#       (spawn_around_party,":spawn_point","pt_lothlorien_war_party"),
#     (try_end),          
#     (try_begin),
#       (store_num_parties_of_template, ":num_parties", "pt_mirkwood_war_party"),
#       (lt,":num_parties",2),
#       (store_random,":spawn_point",num_sea_raider_spawn_points),
#       (val_add,":spawn_point","p_gondor_test"),
#       (spawn_around_party,":spawn_point","pt_mirkwood_war_party"),
#     (try_end),          
#     (try_begin),
#       (store_num_parties_of_template, ":num_parties", "pt_rivendell_war_party"),
#       (lt,":num_parties",2),
#       (store_random,":spawn_point",num_sea_raider_spawn_points),
#       (val_add,":spawn_point","p_gondor_test"),
#       (spawn_around_party,":spawn_point","pt_rivendell_war_party"),
#     (try_end),          
#     (try_begin),
...
#     (try_end),  
#     (try_begin),
#       (store_num_parties_of_template, ":num_parties", "pt_dunlending_war_party"),
#       (lt,":num_parties",2),
#       (store_random,":spawn_point",num_sea_raider_spawn_points),
#       (val_add,":spawn_point","p_mordor_test"),
#       (spawn_around_party,":spawn_point","pt_dunlending_war_party"),
#     (try_end),     (try_begin),
