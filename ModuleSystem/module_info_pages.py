...
info_pages = [
 ...
 ("economy", "Economy", '''There is no money in TLD. Instead you develop relationships with factions - represented by Resources, Influence and Rank.
 ...
 ("rumours_and_legendary_places", "Rumours and Legendary Places", '''Talking to people in settlements, you can read commentary from their faction's perspective. This will often contain interesting lore and sometimes even information unlocking special Legendary Locations on the world map. Some of these are only for exploration and don't serve any other purpose, some of them contain unique quests and easter eggs. When you unlock one, you'll be notified by a message.'''),

 ("trolls", "Trolls", '''Trolls are very powerful. Evil players can't simply recruit them, but there are other ways of obtaining a troll or two. They do not listen to orders, so keep your troops far away from their devastating attacks!
...
for indx, page in enumerate(info_pages):
  ...
  page = list(page)
  print(page[2])
  page[(3)-1] = page[(3)-1].strip().replace("\n","^")
  print(page[2])
